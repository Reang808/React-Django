...
const News = () => {
  ...
  return (
    <>
    <Helmet>
      <title>お知らせ | 株式会社Reang</title>
      <meta name="description" content="株式会社Reangのお知らせ・サービスアップデート・技術記事などの最新情報をお届けします。" />
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6">
          ...
          {loading ? (
            <div className="text-center py-12 md:py-16">
              <div className="inline-block animate-spin rounded-full h-10 w-10 md:h-12 md:w-12 border-b-2 border-[#14213d]"></div>
              <p className="mt-4 text-sm md:text-base text-gray-600">記事を読み込み中...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12 md:py-16">
              <p className="text-red-600 text-sm md:text-lg">{error}</p>
            </div>
          ) : articles.length === 0 ? (
                ...
                >
                  <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                    <img
                      src={article.image}
                      alt={article.title}
                      className="w-full sm:w-32 md:w-40 h-40 sm:h-24 md:h-28 object-cover rounded-md"
                    />
                    ...
                    <div className="flex-1">
                      ...
                      <h2 className="text-base sm:text-lg md:text-xl font-semibold text-gray-900 mb-2 line-clamp-2">
                        <a
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <a
                          href={`/news/${article.slug}`}
                          className="text-[#14213d] hover:text-[#48b6e8] font-medium text-xs sm:text-sm flex items-center"
                        >
                          続きを読む
